...
function getEntryImage(siteId: string): string {
  const site = heritageSites.find((s) => s.id === siteId);
  if (site?.image) return site.image;
  const place = places.find((p) => p.id === siteId);
  return place?.image || '';
}
            ...
            {passportStamps.map((entry, i) => (
              ...
              >
                  ...
                  <Card hover padding="lg" className="text-center h-full">
                    {(() => {
                      const img = getEntryImage(entry.siteId);
                      return img ? (
                        <img src={img} alt={entry.siteName} className="w-16 h-16 rounded-2xl object-cover mx-auto mb-4" loading="lazy" />
                      ) : (
