...
function getSiteImage(siteId: string): string {
            ...
            {passportStamps.map((entry, i) => (
              ...
              >
                  ...
                  <Card hover padding="md" className="flex items-center gap-4">
                    {(() => {
                      const img = getSiteImage(entry.siteId);
                      return img ? (
                        <img src={img} alt={entry.siteName} className="w-12 h-12 rounded-xl object-cover shrink-0" loading="lazy" />
                      ) : (
